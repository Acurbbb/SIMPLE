...
addpath your_NIFTI_path;
addpath ./mex;
tsr = 1 % time scaling, if higher activity is desired 
is_rej = true % geometric rejection

% path of source file
src_folder = 'input_your_folder_here'
src_files = dir(fullfile(src_folder, '*_singles.lm'));

% processing of lesion file in ANALYZE format
file_lesion  = './mouse_lesion.hdr';
le = load_untouch_nii(file_lesion);
vsize = le.hdr.dime.pixdim(2:4);
lssize = le.hdr.dime.dim(2:4);
lsmask = double(le.img);

% get the position of 3D image
sPos = [0, 0, 0];
origin = -lssize.*vsize/2;
fprintf('Center of phantom: %.1f %.1f %.1f mm\n', sPos(1), sPos(2), sPos(3));

% FOV, used to reject events
...
opt_time.direct = 0.4*10^-9; % lifetime of direct annihilation
shift_en = 0.53;             % above which are shifted. Note this energy is unblurred
...
% open output file
save_dir = fullfile(parent_folder, ['tric_SPLIT_rej_pa_no_aa_tsr', num2str(tsr),'x_en', num2str(annihEn(1)),'-',num2str(annihEn(2)),'_',num2str(pGammaEn(1)),...
...
if isempty(p) 
    parpool(n_workers);
elseif ~isempty(n_workers) && p.NumWorkers ~= n_workers
    delete(p);
    parpool(n_workers);
end
tic
...
n_it = ceil(length(src_files) / max_job)
for it = 1:n_it
...
function sgl = keep_unscattered(sgl, id)
flag = sgl(id.cp, 1:3:end-2) == 0 & sgl(id.cp, 2:3:end-1) == 0;
sgl = reshape(sgl, size(sgl,1)*3, []);
sgl = sgl(:, flag);
sgl = reshape(sgl, size(sgl,1)/3, []);
end

function sgl = read_sgl_data(src_files, nrow, dtype)
    sgl = cell(1, length(src_files));
    for iw = 1:length(src_files)
        sgl{iw} = reshape(touch(fullfile(src_files(iw).folder, src_files(iw).name), dtype), nrow, []);
    end
        
%     seg = zeros(n_worker, 2); 
%     if dtype == 'double'
%         size_of_dtype = 8;
%     else
%         error('')
...
%         if iw == 1
%             seg(iw,1) = 1;
%             seg(iw,2) = src_files(1).bytes/size_of_dtype/nrows;
%         else
%             seg(iw,1) = seg(iw-1,2)+1;
%             seg(iw,2) = seg(iw-1,2)+src_files(iw).bytes/size_of_dtype/nrows;
%         end
%     end
%     
%     sgl = zeros(nrows, seg(end,2));
%     
%     for iw = 1:n_worker
%         sgl(:, seg(iw,1):seg(iw,2)) = reshape(touch(fullfile(src_dir, src_files(iw).name), dtype), nrow, []);
%     end
end
